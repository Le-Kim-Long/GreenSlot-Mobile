...
import { ChevronRight, Grid3X3, MapPin, Search, SlidersHorizontal, Sprout } from 'lucide-react-native';
...
export default function GardenListScreen({ navigation }: CustomerTabProps<'Gardens'>) {
  ...
  return <SafeAreaView style={styles.safe} edges={['top']}>
    ...
    <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.chips} keyboardShouldPersistTaps="handled">
      <TouchableOpacity style={[styles.chip, !locationId && styles.chipActive]} onPress={() => setLocationId(undefined)}><Grid3X3 size={15} color={!locationId ? colors.white : colors.green[600]} /><Text style={[styles.chipText, !locationId && styles.chipTextActive]}>Tất cả cơ sở</Text><Text style={[styles.chipCount, !locationId && styles.chipCountActive]}>{slots.length}</Text></TouchableOpacity>
      {locations.map(location => <TouchableOpacity key={location.id} style={[styles.chip, locationId === location.id && styles.chipActive]} onPress={() => setLocationId(location.id)}><MapPin size={15} color={locationId === location.id ? colors.white : colors.green[600]} /><Text style={[styles.chipText, locationId === location.id && styles.chipTextActive]}>{location.name}</Text></TouchableOpacity>)}
    </ScrollView>
